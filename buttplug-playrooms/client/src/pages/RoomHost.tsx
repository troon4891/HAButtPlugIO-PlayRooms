  ...
  if (loading || !room) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-slate-400">Loading room...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen p-4 lg:p-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-sm text-slate-400">
              ...
              <span className="flex items-center gap-1">
                <Users className="w-3 h-3" /> {guests.length}/{room.maxGuests} guests
              </span>
            </div>
          </div>
        </div>
      ...
      {pendingGuests.length > 0 && (
        <div className="card mb-4 border-yellow-600">
          <h3 className="text-sm font-medium text-yellow-400 mb-2">Pending Guests</h3>
          <div className="space-y-2">
            {pendingGuests.map((g) => (
              <div key={g.guestId} className="flex items-center justify-between bg-slate-700 rounded-lg px-3 py-2">
                <div>
                  <span className="font-medium">{g.name}</span>
                  {g.code && <span className="ml-2 text-sm text-slate-400">Code: {g.code}</span>}
                </div>
                <div className="flex gap-2">
                  <button onClick={() => approveGuest(g.guestId)} className="btn-primary text-sm px-3 py-1">Approve</button>
                  <button onClick={() => rejectGuest(g.guestId)} className="btn-danger text-sm px-3 py-1">Reject</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {guests.length > 0 && (
        <div className="card mb-4">
          <h3 className="text-sm font-medium text-slate-300 mb-2">Connected Guests</h3>
